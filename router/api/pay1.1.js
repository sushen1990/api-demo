...
router.get("/getOrderInfo", (req,res)=>{
	...
	const out_trade_no = Date.now().toString(); //商户唯一订单号
	const total_amount = "0.01"; // 付款金额
...
// 3、生成签名
	function _buildSign(paramsMap) {
		...
		let privateKey = fs.readFileSync(APP_PRIVATE_KEY_PATH, 'ascii');
...
// 4 加密签名
	...
	SIGN = [...params].map(([k, v]) => `${k}=${encodeURIComponent(v)}`).join('&');
	console.log(SIGN)
	if(SIGN == ""){
